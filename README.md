...
- 패널 최소화/복원: 갤러리 아이콘 버튼, 상태 유지
- 패널 이동: 헤더 드래그 또는 최소화 아이콘 드래그로 위치 변경
- 검색어 입력: 현재 탭에서 이미지 검색으로 이동
- 검색 후 자동 수집: 체크 시 검색 결과에서 자동 수집 시작
...
- 디버그 로그: “실패 URL 로그 출력” 체크 시 콘솔에 상세 로그 출력
- 썸네일 차단: `tbn0.gstatic.com` / `ogw` 썸네일 도메인은 자동 필터링
- 검색어: 입력 후 검색 버튼 또는 Enter로 이미지 검색 이동
- 검색 후 자동 수집: 패널에서 검색한 경우에만 자동 수집 시작
